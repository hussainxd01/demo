...
import { ChevronLeft, ChevronRight, Edit2, Trash2, X, AlertTriangle } from "lucide-react";
...
export default function DataTable({
  ...
  const [deleteModal, setDeleteModal] = useState({ isOpen: false, itemId: null, itemName: null });

  const handleDeleteClick = (row) => {
    const displayName = row.name || row.email || row._id?.slice(-8).toUpperCase() || "this item";
