...
export default function EditProductPage({ params }) {
  ...
  const handleImagesChange = (e) => {
    ...
    const totalImages = existingImages.filter(img => !imagesToRemove.includes(img)).length + images.length + fileList.length;
    
    if (totalImages > 5) {
      setErrors((prev) => ({ ...prev, images: `Maximum 5 images allowed (you have ${existingImages.filter(img => !imagesToRemove.includes(img)).length} existing + ${images.length} new)` }));
      return;
    }
  ...
  const validate = () => {
    ...
    if (totalImages === 0) nextErrors.images = "Please keep at least 1 image or add new ones";
  ...
  const buildPayload = () => {
    ...
    const dimensions = {
      ...(form.shippingLength !== "" && { length: Number(form.shippingLength) }),
      ...(form.shippingWidth !== "" && { width: Number(form.shippingWidth) }),
      ...(form.shippingHeight !== "" && { height: Number(form.shippingHeight) }),
    };
              ...
              <div>
                ...
                >
                  ...
                  {existingImages.length > 0 && (
                    <div className="mt-5">
                      <p className="text-xs text-gray-600 mb-3">Existing images</p>
