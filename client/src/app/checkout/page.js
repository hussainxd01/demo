...
import { ArrowLeft, Loader, AlertCircle, CheckCircle, CreditCard, Banknote, ShieldCheck } from "lucide-react";
...
export default function CheckoutPage() {
  ...
  const [orderData, setOrderData] = useState({
    shippingAddress: {},
    paymentMethod: "razorpay", // Default to online payment
    giftWrap: false,
    notes: "",
  });
      ...
      if (orderData.paymentMethod === "razorpay") {
        ...
        await paymentService.initiatePayment({
          ...
          onSuccess: (result) => {
            ...
            setTimeout(() => {
              router.push(`/account/orders/${result._id}?new=true`);
            }, 2000);
          },
          onFailure: (err) => {
            setPaymentProcessing(false);
                  ...
                  <div className="bg-gray-50 rounded-lg p-6 text-left space-y-3 mb-6">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className={`font-semibold capitalize ${
                        orderSuccess.paymentStatus === "completed" 
                          ? "text-green-600" 
                          : "text-yellow-600"
                      }`}>
                        {orderSuccess.paymentStatus === "completed" ? "Paid" : "Pending"}
                      </span>
                    </div>
                    <div className="flex justify-between">
            ...
            {step === 3 && (
              <div className="space-y-6">
                ...
                <button
                  ...
                  className={`w-full px-6 py-4 rounded-lg transition-colors font-bold disabled:opacity-50 flex items-center justify-center gap-2 ${
                    orderData.paymentMethod === "razorpay"
                      ? "bg-green-600 text-white hover:bg-green-700"
                      : "bg-amber-400 text-gray-900 hover:bg-amber-500"
                  }`}
                >
                  {(isLoading || paymentProcessing) && <Loader className="w-5 h-5 animate-spin" />}
                  {paymentProcessing 
                    ? "Processing Payment..." 
                    : isLoading 
                      ? "Creating Order..." 
                      : orderData.paymentMethod === "razorpay"
                        ? `Pay Rs. ${(total + (orderData.giftWrap ? 100 : 0)).toFixed(2)}`
                        : "Place Order (COD)"
                  }
                </button>
